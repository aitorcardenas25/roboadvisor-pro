...
import { UpdateNewsletterSchema } from '@/lib/schemas';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const v = await validateBody(req, UpdateNewsletterSchema);
  if (!v.ok) return v.response;

  const { id } = await params;

  if (v.data.status) {
    const updated = await changeStatus(id, v.data.status);
  ...
  const updated = await updateNewsletter(id, v.data as Parameters<typeof updateNewsletter>[1]);
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isAdmin(session)) return NextResponse.json({ error: 'No autoritzat.' }, { status: 403 });
  const { id } = await params;
  if (!await deleteNewsletter(id)) return NextResponse.json({ error: 'No es pot eliminar.' }, { status: 422 });
